...
# use 3d slicer to create the mask (ROI) from atlas 
# or copy the existing mask in 'mask' folder


#%% transform, crop and resample and crop image to the same size in 3D slicer

# use lead dbs to register the atlas to the image (reg folder as the project folder)
# do not use lead dbs to register the PET images to correct motion, the registration is not accurate!
# (use transformation matrix from lead dbs to transform the atlas to the PET/CT image space) 
# transformation matrix from normalization/transformation folder in lead dbs, eg:
# sub-200169M6_from-D99v2_to-anchorNative_desc-ants.nii.gz (from standard space to anchor space)

# Transform the atlas and mask to anchor space.
# note: first resample to the same resolution (
    # use resample_all_volumes_to_resolution2self_nearest(resolution=[1, 1, 1]) for the atlas and mask in slicer
    # use resample_all_volumes_to_resolution2self_linear(resolution=[1, 1, 1]) for the image in slicer)
# then crop to the same size (
    # use the Crop_150_150_100.mrk.json file to crop the images to the same size
    # use auto_crop_volumes2self_nearest() for the atlas and mask
    # use auto_crop_volumes2self_linear() for the image)
# 
...
pet_folder = os.path.join(resample_folder, pet_basename)
...
k2p = k2p_list[rf'{pet_basename}_mrtm_k2p.nii.gz']
r1 = r1_list[rf'{pet_basename}_mrtm_r1.nii.gz']
...
srtmb_asl_basis_bp_files = os.path.join(vol_param_folder, rf'{pet_basename}_srtmb_asl_basis_bp.nii.gz')
srtmb_k2p_basis_bp_files = os.path.join(vol_param_folder, rf'{pet_basename}_srtmb_k2p_basis_bp.nii.gz')
mrtm_k2p_basis_bp_files = os.path.join(vol_param_folder, rf'{pet_basename}_mrtm_k2p_bp.nii.gz')
srtm_bp_files = os.path.join(vol_param_folder, rf'{pet_basename}_srtm_bp.nii.gz')
srtm_basis_bp_files = os.path.join(vol_param_folder, rf'{pet_basename}_srtm_bp.nii.gz')
mrtm_bp_files = os.path.join(vol_param_folder, rf'{pet_basename}_mrtm_bp.nii.gz')
